...
export function LandingSection() {
    return <div className="flex flex-col items-center sm:items-start">
        <div className={"sm:flex sm:flex-row-reverse sm:w-dvw sm:pr-50 sm:justify-between mt-2 sm:mt-0"}>
            <div className={"flex bg-primary rounded-full w-fit h-fit aspect-square p-2 pb-0 overflow-clip mx-20 mb-2"}>
                <Image src={"/melanie.png"} alt={"Picture of Melanie"} width={319} height={547}
                       style={{objectFit: "cover", objectPosition: "top"}}/>
            </div>
            <div className={"flex flex-col items-center sm:items-start"}>
                <h2 className={"font-bold text-foreground text-5xl mb-2 mt-2"}>
                    Hi, I&apos;m
                    {" "}
                    <span className={"text-primary"}>Melanie</span>
                </h2>
                <div className={"min-h-36 md:min-h-fit"}>
                    <div className={"flex flex-row flex-wrap font-bold text-accent mb-4 text-2xl"}>
                        ...
                        <TypingText
                            ...
                            text={[
                                ...
                                "My best projects are with friends, dice, and a D&D character sheet",
                ...
                <p
                    className={"bg-tag text-tag-foreground px-2 py-1 mb-4 max-w-fit"}
                    style={{borderRadius: "15px"}}>Available for opportunities
                </p>
                <Button asChild>
                    <Link href={"/#projects"} className={"bg-primary text-white p-2"}>View my
                        projects</Link>
                </Button>
            </div>
        </div>
